...
def calcularPrecio(tarifa, tiempoDeReservacionr):
        ...
        if (minutosNormales % 60 == 0):
            ...
            costonormal = (minutosNormales // 60) * tarifa.tasaDiaSemana;
        
        else:
            
            costonormal = ((minutosNormales // 60) + 1) * tarifa.tasaDiaSemana;
            
        if (minutosFinDeSemana % 60 == 0):
            
            costoFinDeSemana = (minutosFinDeSemana // 60) * tarifa.tasaFinSemana;
        
        else:
            
            costoFinDeSemana = ((minutosFinDeSemana // 60) + 1) * tarifa.tasaFinSemana;            
        
        resultado = Decimal( costonormal + costoFinDeSemana).quantize(Decimal('1.00'))            
